...
<form class="wiki-search wiki-compact-search" method="get" action="<?php echo esc_url( $wiki_saved_search_action ); ?>">
    <label class="wiki-search-field">
        <input type="search" name="s" value="<?php echo esc_attr( $wiki_saved_search ); ?>" autocomplete="off" placeholder="<?php esc_attr_e( 'Search saved articles', 'wordopedia' ); ?>" aria-label="<?php esc_attr_e( 'Search saved articles', 'wordopedia' ); ?>">
    </label>
